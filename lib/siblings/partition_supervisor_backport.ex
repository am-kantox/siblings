unless Code.ensure_loaded?(PartitionSupervisor) do
  defmodule PartitionSupervisor do
    ...
    def start_link(opts) when is_list(opts) do
      ...
      children =
        for partition <- 0..(partitions - 1) do
          ...
          [start_child_args] = args
          start_child_args = Keyword.put_new(start_child_args, :strategy, :one_for_one)

          start = {__MODULE__, :start_child, [mod, fun, [start_child_args], name, partition]}
    ...
    defp whereis_name(name, partition) when is_pid(name) do
      ...
      |> list_first(:undefined)
    end

    @spec list_first([], any) :: any
    @spec list_first([elem, ...], any) :: elem when elem: var
    def list_first(list, default \\ nil)
    def list_first([], default), do: default
    def list_first([head | _], _default), do: head
